...
# 定义节点以及边
graph_builder.add_node("1", lambda state: chatbot(state, "B1：我会输入体征，请从呼吸系统角度根据狗生命体征进行约200tokens的简短推理，输出一个最可能症状，症状必须来源于狗的常见症状而非生命体征，输出格式如下：'B1:从呼吸系统角度...'"))
graph_builder.add_node("2", lambda state: chatbot(state, "B2：我会输入体征，请从体温代谢角度根据狗生命体征进行约200tokens的简短推理，输出一个最可能症状，症状必须来源于狗的常见症状而非生命体征，输出格式如下：'B2:从体温代谢角度...'"))
graph_builder.add_node("3", lambda state: chatbot(state, "B3：我会输入体征，请从血液循环免疫系统角度根据狗生命体征进行约200tokens的简短推理，输出一个最可能症状，症状必须来源于狗的常见症状而非生命体征，输出格式如下：'B3:从血液循环免疫系统角度...'"))
graph_builder.add_node("4", lambda state: chatbot(state, "C1：为B1 B2 B3分别进行总结摘要，简短输出共128tokens的三段摘要，输出格式如下：'C1：1.B1总结了...2.B2总结了...3.B3总结了...4.总之该狗患有...症状'"))
# 定义边
